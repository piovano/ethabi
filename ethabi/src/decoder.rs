...
fn decode_param(param: &ParamType, slices: &[[u8; 32]], offset: usize) -> Result<DecodeResult, Error> {
    match *param {
        ...
        ParamType::Array(ref t) => {
            ...
            let sub_slices = &slices[new_offset..];
            let mut sub_offset = 0;
            for _ in 0..len {
                let res = try!(decode_param(t, &sub_slices, sub_offset));
                sub_offset = res.new_offset;
                tokens.push(res.token);
            }
        ...
        ParamType::FixedArray(ref t, len) => {
            let result = if t.is_dynamic() {
                let offset_slice = try!(peek(slices, offset));
                let new_offset = (try!(as_u32(offset_slice)) / 32) as usize;
        ...
        ParamType::Tuple(ref ts) => {
            let is_dynamic = ts.iter().find(|t| t.is_dynamic()).is_some();
...
mod tests {
    ...
    fn decode_static_tuple() {
        ...
        let address1 = Token::Address([0x11u8; 20].into());
        let address2 = Token::Address([0x22u8; 20].into());
        let array = Token::FixedArray(vec![address1, address2]);
        let tuple = Token::Tuple(vec![uint, array]);
    ...
    fn decode_dynamic_tuple() {
        let encoded = hex!("
            ...
            70696f76616e6f00000000000000000000000000000000000000000000000000
        ");
        let address = Token::Address([0x11u8; 20].into());
        let addresses = Token::Array(vec![address]);
        let string = Token::String("piovano".to_owned());
        let tuple = Token::Tuple(vec![addresses, string]);
        let expected = vec![tuple];
