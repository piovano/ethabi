...
mod tests {
	...
	fn param_type_deserialization() {
		let s = r#"["address", "bytes", "bytes32", "bool", "string", "int", "uint", "address[]", "uint[3]", "bool[][5]", "(uint[],bytes)"]"#;
		...
		assert_eq!(deserialized, vec![
			...
			ParamType::Tuple(vec![ParamType::Array(Box::new(ParamType::Uint(256))), ParamType::Bytes])
		]);
	}
}
